...
// Переменная для предотвращения множественных перезапусков
let isRestarting = false;

/**
 * Перезапускает игру после Game Over.
 * Сбрасывает счет, положение мяча, создает новые кирпичи и запускает игровой цикл.
 */
function restartGame() {
  if (isRestarting) return; // Предотвращаем повторный вызов
  isRestarting = true;
  toggleLoader(true, 'ПЕРЕЗАПУСК...');

  setTimeout(() => {
    location.reload(); // Перезагружаем страницу для чистого старта
  }, 1000);
}
...
function updateGame() {
  ...
  if (ball.y + ball.radius > paddle.y && ball.x > paddle.x && ball.x < paddle.x + paddle.width && ball.vy > 0) {
    ...
    ball.vx += hitPos * 10;
    // Звуковой эффект отскока от платформы
    window.soundManager?.playPaddleBounce();
  }
  ...
  for (let i = bricks.length - 1; i >= 0; i--) {
    ...
    if (ball.x > b.x && ball.x < b.x + b.width && ball.y > b.y && ball.y < b.y + b.height) {
      ...
      // Звуковой эффект попадания по кирпичу
...
function gameOverFunc() {
  ...
  // Звуковой эффект окончания игры
